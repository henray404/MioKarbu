...
class Motor:
        ...
        if self.track is not None:
            ...
            if result['out_of_bounds']:
                ...
                if abs(self.velocity) > self.physics.config.wall_explode_speed:
                    if not self.invincible: self.alive = False; self.is_alive = False
                    else: self.physics.state.velocity *= -0.2; self.x, self.y = prev_x, prev_y
                else: self.physics.state.velocity *= -0.4; self.x, self.y = prev_x, prev_y
            elif result['slow_zone']: self.physics.state.velocity *= 0.99
    ...
    def draw(self, screen, camera_or_x, camera_y: int = None) -> None:
        if camera_y is not None: cam_x, cam_y = camera_or_x, camera_y
        else: cam_x, cam_y = camera_or_x.x, camera_or_x.y
